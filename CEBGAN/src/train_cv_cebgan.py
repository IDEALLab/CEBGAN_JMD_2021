...
if __name__ == '__main__':
    ...
    batch = 196 #[36, 64, 96, 128, 250, 400]
    epoch = 10500 #[2000, 3500, 5250, 7000, 14000, 21000]
    save_intvl = 1000
    kf = KFold(n_splits=4)
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(aoas_opt)):
        ...
        def metrics(epoch, generator, writer, *args, **kwargs):
            if (epoch + 1) % 100 == 0 or epoch == 0:
                ...
                inp_paras_train_n = (inp_paras_train - mean_std[0]) / mean_std[1]
                inp_paras_test_n = (inp_paras_test - mean_std[0]) / mean_std[1]

                X_train = np.hstack([airfoils_train.reshape(airfoils_train.shape[0], -1), aoas_opt_train, inp_paras_train_n])
                X_test = np.hstack([airfoils_test.reshape(airfoils_test.shape[0], -1), aoas_opt_test, inp_paras_test_n])
                train_mean, train_std = ci_mmd(n_run, build_gen_func(inp_paras_train_n), X_train)
                test_mean, test_std = ci_mmd(n_run, build_gen_func(inp_paras_test_n), X_test)
                
                writer.add_scalar('Metric/MMD_train', train_mean, epoch)
                writer.add_scalar('Error/MMD_train', train_std, epoch)
                writer.add_scalar('Metric/MMD_val', test_mean, epoch)
                writer.add_scalar('Error/MMD_val', test_std, epoch)
        ...
        tb_dir = os.path.join(save_dir, 'runs/batch', 'b{}_e{}_l{}'.format(batch, epoch, egan_cfg['lamb']), 'fold_{}'.format(fold))
